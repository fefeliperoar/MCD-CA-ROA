...
escena.background = new THREE.Color(0x0008ff);
...
const grilla = new THREE.GridHelper(50, 50, 0x888888, 0x888888);
...
// El color base se mezcla hacia naranjo a medida que sube la amplitud.
const colorBaseModulo = new THREE.Color(0xd7d2c8);
const colorAmplitudAlta = new THREE.Color(0xff6a00);
const amplitudMaxima = 6;

function actualizarColorModulo() {
  const t = THREE.MathUtils.clamp(parametros.amplitud / amplitudMaxima, 0, 1);
  materialModulo.color.copy(colorBaseModulo).lerp(colorAmplitudAlta, t);
}
